...
export async function countSegment(
  ...
  try {
    ...
    if (segment === 'no_labs') {
      const { data: allIds } = await adminDb.from('profiles').select('id')
      const { data: labIds } = await adminDb.from('biomarkers_static').select('user_id')
    ...
    if (segment === 'custom') {
      ...
      if (f.has_labs === false) {
        let { data: base } = await adminDb.from('profiles').select('id')
        ...
        if (f.onboarding_incomplete) {
          const { data: inc } = await adminDb.from('profiles').select('id').eq('onboarding_completed', false)
          const incSet = new Set((inc ?? []).map(p => p.id))
          result = result.filter(p => incSet.has(p.id))
        }
        if (f.active_7d) {
          const cutoff = new Date(Date.now() - 7 * 86400 * 1000).toISOString()
          const { data: act } = await adminDb.from('biomarkers_static').select('user_id').gte('created_at', cutoff)
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      let q: any = adminDb.from('profiles').select('id', { count: 'exact', head: true })
