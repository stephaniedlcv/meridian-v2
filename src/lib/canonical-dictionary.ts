...
export const CANONICAL_DICTIONARY: Record<string, CanonicalMarker> = {
  ...
  amylase: {
    ...
    system: 'digestive',
    riskProfile: 'u-shaped',
    normalF: { min: 30, max: 110 },
    normalM: { min: 30, max: 110 },
    optimalF: { min: 35, max: 90 },
    optimalM: { min: 35, max: 90 },
    impossibleMin: 0,
    impossibleMax: 5000,
    priorityWeight: 2,
  },
  lipase: {
    ...
    system: 'digestive',
    riskProfile: 'linear-high',
    normalF: { min: 23, max: 300 },
    normalM: { min: 23, max: 300 },
    optimalF: { min: 23, max: 160 },
    optimalM: { min: 23, max: 160 },
    impossibleMin: 0,
    impossibleMax: 10000,
    priorityWeight: 2,
  },
...
// Used as Tier-2 fallback when PDF extraction does not provide a range.
export function getCanonicalFallbackRange(
  slug: string,
  biologicalProfile: 'female' | 'male'
): { min: number; max: number } | null {
  const marker = CANONICAL_DICTIONARY[slug]
  if (!marker) return null
  const r = biologicalProfile === 'female' ? marker.normalF : marker.normalM
